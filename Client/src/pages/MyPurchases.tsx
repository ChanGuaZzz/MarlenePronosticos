...
function MyPurchases() {
  ...
  const [selectedIsGiftCard, setSelectedIsGiftCard] = useState(false);
  const [selecterdGiftCardCode, setSelectedGiftCardCode] = useState<string | undefined>(undefined);
  useEffect(() => {
    ...
    const fetchPurchases = () => {
      ...
      axios
        .get(`${import.meta.env.VITE_URL_SERVER}/getpurchases`, { withCredentials: true })
        .then((response) => {
  ...
  const handleShowForecast = (isGiftCard: boolean, GiftCardCode?: string, imageUrl?: string) => {
    
    setSelectedImage(imageUrl || "");
    setSelectedIsGiftCard(isGiftCard);
  ...
  return (
    <>
                      ...
                      <div className="p-6">
                        ...
                        {purchase.productId&&!purchase.productId.isGiftCard && (
                          <div className="flex items-center text-sm text-gray-500 mb-2">
                            ...
                            Partido: {purchase.productId.matchDate && new Date(purchase.productId.matchDate).toLocaleDateString()}
                          </div>
                        )}
      ...
      <ForecastModal
        isOpen={showModal}
        isGiftCard={selectedIsGiftCard}
        giftCardCode={selecterdGiftCardCode}
