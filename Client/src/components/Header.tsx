...
const Header: React.FC = () => {
  ...
  return (
        ...
        <ul className="px-4 pt-2 pb-4 space-y-2 bg-gradient-to-r from-green-900 via-green-800 to-green-700">
          {navItems.map((item) => (
            <li key={item.path}>
              <Link 
                to={item.path} 
                className={`block py-2 px-3 rounded ${isActive(item.path) ? 'bg-green-800 text-green-200' : 'text-white hover:bg-green-800 hover:text-green-200'}`}
    ...
    </header><div style={{ paddingTop: "60px" }}>
    </div></>
  );
};
