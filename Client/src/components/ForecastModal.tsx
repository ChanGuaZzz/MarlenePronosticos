...
const ForecastModal: React.FC<ForecastModalProps> = ({ isOpen, imageUrl, onClose, isGiftCard, giftCardCode }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-auto bg-black/60 backdrop-blur-lg bg-opacity-75 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-lg max-w-4xl mx-auto">
        ...
        <div className="p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">{!isGiftCard ? "Pronóstico" : "Codigo"} </h3>
          <div className="mt-2 relative select-none touch-none">
            ...
            {!isGiftCard ? (
              ...
              <div className="bg-white w-full rounded-lg shadow-lg py-2 text-center">
                <h3 className="text-lg font-medium text-gray-900 ">Tu código de regalo es:</h3>
                <p className=" w-full text-[80%] sm:text-xl px-3 text-wrap font-bold text-green-600 underline ">{giftCardCode}</p>
                <p className="text-sm text-gray-500 ">¡Disfruta de tu regalo!</p>
              </div>
            )}
          </div>
          <div className="mt-5 text-center">
            {!isGiftCard&&<p className="text-xs text-red-600 mb-2">⚠️ Capturas de pantalla están prohibidas. Contenido de pago.</p>}
            <button
              type="button"
